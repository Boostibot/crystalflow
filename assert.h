...
#define TEST(x, ...)                            (!(x) ? (assertion_report(#x, __LINE__, __FILE__, __FUNCTION__, "" __VA_ARGS__), (void) sizeof printf(" " __VA_ARGS__), abort()) : (void) 0)
#define _DISSABLED_TEST(x, ...)                 ((void) sizeof printf(" " __VA_ARGS__), (void) sizeof (x))
...
#ifndef platform_assume_unreachable
    #define platform_assume_unreachable() (void) 0
#endif
...
void assertion_report(const char* expression, int line, const char* file, const char* function, const char* format, ...);
