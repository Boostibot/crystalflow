...
def step():
    ...
    for i in range(0, N):
        ...
        dt_ro = -(fe_upw_ro*fe_upw_ux - fw_upw_ro*fw_upw_ux)/dx 

        ax = 0
        # ro_dt_ux = \
            # -(fe_avg_ux - fw_avg_ux)/dx \
            
        ro_dt_ux = \
            -(fe_upw_ro*fe_upw_ux*fe_avg_ux - fw_upw_ro*fw_upw_ux*fw_avg_ux)/dx \
            +(lam + 2*mu)*(fe_der_ux - fw_der_ux)/dx \
            -ux*dt_ro + ro*ax \
            -R_spec*T*(fe_upw_ro - fw_upw_ro)/dx 
...
def graph():
    ...
    ux_cells, = ax_ux.plot(cell_centers, cells_ux, 'o-', label='ux cells')
    ux_faces, = ax_ux.plot(face_positions, faces_avg_ux, 's', label='ux faces')

    ax_ux.set_ylim(0, 2)
    ax_ux.set_xlabel('Position')
    ax_ux.set_ylabel('Value')
    ax_ux.legend()
    ax_ux.grid(True)

    ro_cells, = ax_ro.plot(cell_centers, cells_ro, 'o-', label='ro cells')
    ro_faces, = ax_ro.plot(face_positions, faces_avg_ro, 's', label='ro faces')
    ax_ro.set_ylim(0, 2)
    ax_ro.set_xlabel('Position')
